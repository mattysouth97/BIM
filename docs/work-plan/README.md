...
| [P2-25](./items/P2-25-vworld-building-layer-footprint.md) | VWorld building-layer footprint (LT_C_SPBD) — true outline + storey count (height tier VOID) | geometry | M | UC-01, UC-05 | 🔎 |
...
|---|---|---|---|
| 2026-09-04 | P2-31 | **Done.** `useBuildingParcel` fetches the lot with a new `layer=parcel` mode on `/api/vworld/footprint`, so `chooseSetbackFace` finally sees real directional slack — until now `useLedgerReconstruction` passed `parcel: null` and the 일조권 rule was reachable in tests only. The wiring exposed a stated-versus-assumed violation this item had created: with no lot, the setback was handed the site square solved from 대지면적 (own note: 실제 필지 형상이 아님), and an off-centre or L-shaped footprint shows slack against it — so the ledger published "필지에서 south 측으로 1.7 m의 여유가 **관측**되어…" citing `SRC-GIS-PARCEL` for a parcel nobody fetched. Only an observed lot reaches the setback now. `08da81d` | claude-opus-5-session (violet) |
| 2026-09-04 | P2-29, P2-30 | **Sign-off withheld.** `deriveRoomElements` (`bim/derive/twin-elements.ts:285`) computes `footprintArea(recipe)` once, outside the per-floor map, so every storey reports the building footprint and `floor.plate` is ignored. Measured on 서울청운초등학교 (pk `1002122071`): 7 rooms × 2,749.71 = 19,247.6 m² against a **stated** 연면적 of 12,957.58 (+48.5%), and the energy intensity divides by it — 51.2 vs 76.1 kWh/m²·yr. Corrects the initial diagnosis: the reconstruction is NOT the source and needs no change — with no 층별개요 it divides the stated 연면적 across the storeys and returns 12,957.1 against 12,957.58. P2-29 cannot claim one producer and P2-30 cannot claim per-storey envelope while one consumer extrudes the footprint | claude-opus-5-session (violet) |
| 2026-09-04 | P2-25 | Height tier removed, not just documented. `ingest.ts` emitted `{kind:"height", source:"vworld-measured"}` behind a guard no input could satisfy, `HEIGHT_PRIORITY` ranked a tier that could never match, `HEIGHT_SCORE` priced it, and `measuredHeightM` was threaded from `build-engine-input` through `useEngineResult` to two components solely to feed it. All four gone. `"vworld-measured"` stays valid for footprints and storey counts — both genuinely arrive from LT_C_SPBD — and `input-provenance.ts` / `building-geometry.ts` keep their own `measuredHeightM`. The fuse test asserting a ledger-vs-vworld height conflict was pinning the dead tier and now tests ledger-vs-manual. `1208ac4` | claude-opus-5-session (violet) |
| 2026-09-04 | viewer | **Glazing restored on every building.** `seededRandom` in `facade-generator.ts` was a Lehmer step over seeds topping out near 3,000 against a modulus of 2.1 billion — it never wrapped, so the glass/spandrel choice rose monotonically with `floorNo` instead of scattering. Below the crossing every bay was solid, above it every bay was glass. The demo tower had no glass under 14 m; worse, `solidPanelChance` is 0.15 on every non-office recipe, putting the crossing at floor ~48, so every 아파트 / 단독주택 / 근린생활 model rendered a fully solid envelope in every era. Replaced with an imul/xorshift avalanche hash: demo tower 258 → 481 panes, per storey `0 0 0 2 17 39 50…` → `42 49 48 50…`. `f1e08b2` | claude-opus-5-session (violet) |
| 2026-09-04 | P2-25 | **Re-scoped, not retained.** `LT_C_SPBD` carries no building height: verified 34 production buildings (`height: null` in every one, `groundFloors` 28/30) and four upstream bboxes returning the same ten keys, none of them `buld_hg` or `und_flo_co`. Six candidate height-bearing layers eliminated — recorded as "these six do not carry it", NOT as "VWorld has no height". S4 and one acceptance criterion marked VOID; the outline half stands and shipped. `building-geometry.ts` comments now say the middle tier has no supplier. Corrected an overstatement made during triage: the `'measured'` heights provenance grade is NOT unreachable — `ledgerHeit > 0` reaches it constantly, only the `measuredHeightM` disjunct is dead. Genuinely unreachable: `ingest.ts:17`'s `source:"vworld-measured"` height evidence kind. No code behaviour changed; removal of that kind recorded as a deliberate decision for someone else to take | claude-opus-5-session |
| 2026-09-04 | P2-31 | Directional setbacks. New `setback.ts`: `chooseSetbackFace` (용도지역 + lot slack → face) and `insetEdgeToArea` (half-plane clip + bisection to the registered area). `makeLevel` takes the area off ONE face, with a `ConflictEntry` + concentric fallback when a single-face step is geometrically impossible. New `/api/vworld/zoning` route reads `LT_C_UQ111.uname`. **The rule picks the face; 층별개요 picks the amount** — no setback figure from 제86조 is encoded anywhere. Additive `EvidenceInput.parcel` closes the building-outline-XOR-parcel gap | claude-opus-5-session |
