...
| **iso-13790-kernel** (fresh context, bim-72's brief) | own worktree `.claude/worktrees/agent-a4511aa3c99614a0f`, branch `feat/iso-13790-monthly-kernel` off `f5091e7` | `src/lib/energy-standards/monthly-balance.ts` + `__tests__/monthly-balance.test.ts` **only**. Does not touch `src/lib/energy/{heat-loss,annual-demand}.ts`, does not edit `bs-medical-dental-clinic-energy.ts` (hardcodes 267.16 / 2,150.30 / 37.06 / V 20,685.33 with citations until main-coordinator's fields land), wires no route. Not deployed |
