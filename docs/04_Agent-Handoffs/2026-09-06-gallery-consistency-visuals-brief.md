...
| 2 follow-up · FZK contract | bim-54 | `8ad091e` on `lane2-fzk-contract` (on `9820b87`) | FZK's roof `read` was WRONG, not mis-shaped (projected 71.5 per pitch where the others give surface; now `roofs[].surfaceSqm` 85.56) — bim-ae reached the same diagnosis; test message now names the shape it wants. Rounding tolerance derived, n × 0.005, apportioned sectors still 1e-6. U-value rule is provenance: admissible only when `read` names the IFC property/quantity, and a second test proves an uncited thermal figure is still refused. Found by looking: "태양광은 gable 이용률로" — only `flat` had ever been translated; all four typologies now 박공/우진각 etc., pinned. FZK on screen 92.6 kWh/m²·yr, grade 1+. |
| merge 4 | main-coordinator | `74011c5` = shared + `lane1b-fzk-haus` + `lane2-fzk-contract` | tsc 0; **full vitest 4,970 passed / 4 skipped, 418 files.** Four buildings published. |
| 1C · e2e over /models/* | bim-83 | `8674967` on `lane1c-models-e2e` (on `b364ed8`) | **Race closed: does not reproduce** on a fresh Playwright context, first visit, no reload — Clinic 1+ 108.8, apartment 1+++ 40.5, Duplex 1 142.6, each equal to a headless run. What differed when seen: a dev server that had never compiled the route, and a tab never brought to front. Second finding, to 3C: on the apartment the delta strip says "the selected measures move no kWh/m²" under both tracks — a claim about the building where only a claim about the module is supportable; `test.fail()` recorded. Fixed 1A's own miss: `first-door.spec.ts` gallery count (a lane that ran vitest and not Playwright — **any lane that adds a card runs Playwright**). A-WWR-ENGINE-MEAN prose had drifted from the corrected signature (tests fail on a signature change, prose does not) — rewritten with the old figures kept as history. Playwright 63 passed. |
| merge 5 | main-coordinator | `34968d9` + gallery count → 4 with FZK named | tsc 0; vitest reference-buildings/energy/landing 877 passed; Playwright `first-door` + `reference-buildings` specs exit 0. Pushed. **Open lanes (3B, 3C, grade-table item) rebase onto this tip.** |
| grade-table item (assigned 16:05) | bim-54 | pending | Three of four published buildings are dwellings scored on the non-residential table, one band flattering. DECIDED: fix, not disclose — use code chooses the table where one exists; ledger dwellings move too, correctly; office rows must not; before/after on one residential and one office ledger row plus the three dwellings. |
| 3B · merged | bim-7c | `d60171d` (incl. `b7833f4`) on the shared tip | Roof typology read from `energy.roof.{type,read}`; their own blended tilt over the apartment's mixed tile+deck roof had read ~45°, an angle the roof never has (63.2° or 0°) — now resolves to the steep face population, within a few degrees of 63.2. Tested against all four real GLBs; two roof-area artefacts (both-sheets-upward, stacked-layer over-count) documented rather than silent. FZK checked live: flush tilted panels, legend "no HVAC file" distinct from "layer off". |
| merge 6 | main-coordinator | `bf77297` | tsc 0; **full vitest 5,007 passed / 4 skipped, 420 files**; Playwright `reference-buildings` + `first-door` 25 passed. **Task 3's three lanes are all on the shared branch.** |
| 1C · FZK row | bim-83 | `ebaba6f` on `lane1c-models-e2e` | FZK 1+ / 92.6 verified headless before pinning. Services-row assertion was vacuous on an empty layer list — now counted from the manifest (1 / 4 / 7 / 5 across the four; the flow toggle is itself a row, conditionally). **The delta defect is on two of four**: apartment AND FZK start with one unpriced measure already selected unsubsidised, so adding a second the module cannot price fires the zero-delta branch in both states and reports the module's blind spot as physics; the Clinic differs only in starting at 0/6. Two `test.fail()`s with numbers → 3C. |
| merge 7 | main-coordinator | `8b6a414` | tsc 0; the two specs 30 passed (2 expected failures inside). Pushed. **3C and the grade-table item rebase onto this.** |
| 2 · fixtures corrected | bim-54 | `c005747` on `lane2-fzk-contract` | The "341.00 gross / 73.46 glazing" in heat-loss.ts and two tests was an invented fixture wearing the Duplex's ratios (bim-83 caught it); real 340.58 / 64.46, fixtures now read from `duplex-apartment-energy.ts`. A real bug fell out: net weights had been `gross × (1 − wwr)`, which leaves doors in. Note: A-WWR-ENGINE-MEAN exists in TWO files (apartment, Duplex) under one id — bim-54's rewrite is the apartment's, bim-83's the Duplex's; both current. |
| grade table · FIXED | bim-54 | `3b9ff6a` on `lane2-grade-table` | `buildingTypeForGrade(materials, mainPurpsCd?)` replaces `buildingTypeFromMaterials` at all four rating sites. Site kWh/m² unchanged everywhere — a scale, not physics. Apartment 1+++ → **1++**, Duplex 1 → **4**, FZK 1+ → **2**; ledger 공동주택/단독주택 fixtures 1 → **4**; 업무시설 and 의료시설 unchanged. 09000 returns `"default"` from `ledgerUseCategory` and is NOT read as non-residential — falls through to density, disclosure narrowed to that case. A test asserts every moved building moved DOWN (residential bands are tighter; a gain means an inverted mapping). Two old tests asserting the defect rewritten, not loosened. |
| merges 8–9 | main-coordinator | `f36b7d6` + spec pins → 1++ / 4 / 2 | tsc 0; **full vitest 5,023 passed / 4 skipped, 421 files**; `reference-buildings` e2e 20 passed after re-pinning the three dwellings' grades (bim-54 ran vitest, not Playwright — the same miss as 1A's, caught at merge). Pushed. **Only 3C is open.** |
| 3C · the row picks work | bim-24 | `db19b64` on `lane3c-measure-first` (on `8b6a414`) | `MeasureChipRow`: one chip per generated measure, one line each (`외벽 U 0.29 → 0.15 · 427 m² · ₩5120만` — change from the delta, area from the engine's element, cost from `effectiveCapex`); knapsack optimum is a 추천 mark; `ProgramTrackSelector` moved beneath as **지원 재원 / Financing**. `appliedMeasureIds` is the user's set again, seeded once from the first recommendation (`=== null` guard). `evaluateMeasureSet` extracted so a hand-picked set and the optimum are one aggregation. Exclusivity via `measureExclusionKey` in economic-model.ts (**not** measure-interactions.ts, which is damping — brief corrected). Zero-delta branch split `nothing-chosen / only-unpriced / targets-met`; apartment now says the module's own limit, not a building claim. `useProposalVisualIds()` still the single 3D gate, three store tests. Full vitest 5,040. Seen: wall chip → 외벽 −65 W/K; financing chip → NPV ₩1592만 → ₩3496만 with the strip **byte-identical** — financing re-prices, never re-picks. |
| merge 10 | main-coordinator | `8982a7c` | tsc 0; **full vitest 5,056 passed / 4 skipped, 423 files.** `reference-buildings` e2e **6 failed / 14 passed, by premise**: the spec asserts the delta strip changes on a financing click, which 3C makes invariant by design. Pushed with that stated; the rewrite is Lane 1C's (bim-83): measure chip → strip changes; financing chip → NPV/실효 투자비 change, strip unchanged; the two `test.fail()` markers come off. **Every lane's code is on the branch. Remaining: that spec, the final full Playwright run, the browser pass, the deploy.** |

## Browser pass on `8982a7c` (main-coordinator, 16:30–16:50, dev :3000)

Automation tab reports `visibilityState: hidden` on first load (canvas 300×150) and paints on a synthetic resize — item 5, again; every canvas verdict below is after that, and bim-7c/bim-83 saw them in real foreground tabs.

| page | verified | defects routed |
|---|---|---|
| `/models/duplex-apartment` | grade 4 · 142.6; 3/7 chosen = the 추천 set; financing click NPV ₩2269만 → ₩2766만 with the delta strip byte-identical; wall chip 4 → 2 · −50.8, second click restores; PV on the flat roof, low-e glazing; legend one line per fact | measure chip names are the generators' English (`High-Performance Window Replacement` beside `고효율 보일러 교체`) → bim-24. A "70 % prices at 60 %" item was **my error** (the tab had 공공 서울·중앙 persisted; 2256 × 0.3 = 677 exactly) — retracted. |
| `/models/bs-medical-dental-clinic` | 1+ · 108.8; grade sentence names the density-chosen table for 09000; 공공 지자체 → window ₩9197만 → ₩2759만 (30 %), utilisation 113 % → 98 % reconciling with the chip sums; strip identical; PV racked on the flat roof | at 1568×625 the HUD leaves ~110 px of canvas → bim-24 to check at 1366×768 |
| `/models/schependomlaan` | 1++ · 40.5 on the residential table; awaiting badge now says the stand-ins state no envelope direction; flow sentence for six port-less models; zero-delta strip says the module's own limit ("이 실행이 반영할 수 있는 공사가 선택되어 있지 않습니다 — 선택한 1건은 …"); wall chip → 2 chosen, strip moves | **legend heading counts the recommendation (2) while its bullets render the user's set (1)** → bim-7c |
| `/models/fzk-haus` | 2 · 92.6 on the residential table; KIT/IAI attribution; six chips; legend "HVAC: 이 모델에는 해당 계통 파일이 없음" | **measurement notice is the building's English free-text basis paragraph** where the others render the contract's one Korean line → bim-54; MeasureCards show English `name`/`description` → bim-54 to consume bim-24's bilingual source |
| `/building/demo` | 1++ · 71.3; 4/6 chosen; delta −27.9 = 외벽 −1035 + 창호 −2888 + 침기·환기 +37 (−3886, reproduces); roof chip → −29.1 / −4050 and back; 제안 미리보기 OFF hides the proposal; no console errors | wall chip AND window chip both state **2,407 m²** (Duplex: 267 vs 64) → bim-24 to trace which `engineEnvelopeAreas` field each reads; at 1350×583 the HUD covers the whole canvas → same viewport item |

Open at 16:55, all routed with evidence: bim-83 (spec rewrite to 3C's premise + final full Playwright run), bim-24 (bilingual chip names, short-viewport HUD, the 2,407 m² pair), bim-7c (legend count ≠ bullets), bim-54 (FZK's contract notice, card names). Deploy after those land and the final run is green.

**2,407 m² — refuted by bim-24, correctly.** Wall −1035 W/K ÷ (0.58 − 0.15) = 2406.98; window −2888 W/K ÷ (2.10 − 0.90) = 2406.67 — two ΔUs landing on one area is what a real equality looks like; a mis-pointed chip would have printed the wall's area while its W/K came from the true aperture, and they would not reconcile. The demo's curtain wall has WWR 0.5, so `gross − windows` = `windows` = 4,814 / 2 exactly. `measure-claim.test.ts` already pins each branch to its own field on a WWR-0.3 fixture. **Equal numbers on screen are not evidence of a shared source, and unequal ones are not evidence of a correct one; the settling check is reconciliation with a figure computed a different way** (here W/K ÷ ΔU; on the retracted subsidy item, 2256 × 0.3). Two coordinator items in one pass refuted the same way. Also noted: `programTrack` persists across reloads and buildings, so a page can open with a subsidy already applied and nothing saying so — a live hazard for anyone reading prices in a browser pass.
| 3B · legend count | bim-7c | `b61eba2`, merged `1bc0c67` | Header now counts `useEffectiveMeasureIds()` — the same resolved set the bullets render. bim-7c had found it independently on the Clinic while checking the viewer against 3C. Test replicates the real wiring (recommended [HRV, PV], applied [PV] → "1", solar bullet only). Confirmed live: measure chip moves building + legend, financing chip moves neither. bim-7c released, low on context. |
| 1C · spec rewritten to 3C | bim-83 | `9de6cb2`, merged `d300327` (+ one marker removal at merge) | Five tests per building through attributes only: measure chip changes the model and restores; financing re-prices and does not re-pick; legend follows the chosen work; at first load the 추천 chips are the chosen set (the rail answers for the CHOSEN work, not the optimum — brief corrected); the flat-delta case names which kind of nothing via `data-zero-reason`. Three drafts wrong and caught by running: a locator keyed on the attribute the click flips; the Clinic arrives with 0 chosen; financing needs work to re-price. bim-83's expected-fail on the Clinic legend was the defect bim-7c fixed in `b61eba2` one merge earlier — it flipped to an unexpected pass and I removed the marker: **32 passed**. Observation to bim-24: in a fresh context 공공 지자체 leaves the rail byte-identical on all four (FZK's HRV is the discriminating case). |
| FZK notice · **refuted by bim-54** | bim-54 | `12c630c` (test only) | The frame renders `reference-energy-measurement-complete` = "실측 완료 · …" from `measurementState` on FZK exactly as on the Clinic; the English "FULLY MEASURED — …" paragraph is the panel's basis row, where every building's goes. **My page read matched the basis paragraph, not the frame line** — third coordinator item refuted today. The real gap underneath: `basis` strings open with a status clause ("FULLY MEASURED —", "PARTLY PLACEHOLDER —") that restates `measurementState` from a second, unlinked source; they agree on all four by luck. Test landed: frame sentence is one of the contract's two shapes per id, and no basis is short enough to pass for it. **Decided:** strip the status opener from the two basis strings (provenance detail kept) and pin that no basis begins with a status claim — bim-54. |
| 3C · names, viewport, coverage | bim-24 | `b5a68ca`, merged `13871aa` | Bilingual chip names keyed by measure id (`measureDisplayName` in `measure-claim.ts`; Korean = the generators' own where they exist; unknown ids fall back rather than vanish). **공공 지자체 question settled as (a)**: on FZK every chip price moved to exactly 30 % while the rail stayed identical, because the fresh chosen set is PV alone and `subsidyByCategory` omits renewable on purpose — the premise "HRV + PV" was wrong, the reasoning right; `evaluate-measure-set.test.ts` pins the mixed case. New row line when a track covers nothing chosen: "선택한 공사에는 이 지원 재원이 적용되지 않습니다 — 태양광은 신재생에너지 보급사업 소관입니다" (`[data-track-covers-nothing]`). Viewport: each frame slot capped at 38 % and scrolling inside — a cap, not a collapse, because the measure row and the grade sentence are stated facts, not chrome; canvas keeps ≥ a quarter. Rows reproduced the headline again (−547 − 499 − 1871 = −2917). Full vitest 5,066; on the merged tip the model-page spec **32 passed**, tsc 0. bim-24 at ~65 % context. |
...
**Lane 3D landed — main-coordinator, 23:55.** `capexBudgetKrw` is `number | null`, null by default, per building (cleared with the building), never persisted. Null → the recommendation is every NPV-positive measure via `evaluateMeasureSet` and the knapsack is not called; a value → today's knapsack. The slider band is gone from the HUD; the rail's first cell carries an optional 만원 field (`data-twin-budget-input`) and its line says which meaning of 추천 is in force. Consumers handle null (retrofit panel says "예산 없음"; the generative panel's range shows "none (NPV-positive)"). Store tests rewritten to today's truth, not loosened. e2e: the Clinic's fresh chosen set is now its NPV-positive work (PV alone), so a CAPEX track covers nothing on all four — the row was corrected with the reason. Full vitest 5,137 · Playwright 88 · eslint 0 errors.
