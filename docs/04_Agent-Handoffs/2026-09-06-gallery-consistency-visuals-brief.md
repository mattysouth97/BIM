...
| 3B · rebased | bim-7c | `322cbdf` on `lane3b-model-retrofit-visuals` (on `0198b44`), pushed | Viewer now reads `useProposalVisualIds()` so the 제안 미리보기 switch gates the model page's building too; PV count prefers the delta's own `after.materials.renewable.solarPV.capacity` (the kWp the economics prices). Toggle verified live on the Clinic. Full suite 4,828. Holds only for Lane 2's Step 3 sha (two lines in the workspace file). |
