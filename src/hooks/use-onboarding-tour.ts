...
function stepsFromLocalTwinCatalog(language: "ko" | "en"): DriveStep[] {
  return twinTourSteps(language === "ko").map((step) => ({
...
function basicSteps(language: "ko" | "en"): DriveStep[] {
  const local = stepsFromLocalTwinCatalog(language);
  const stepper = local.find((step) => step.element === '[data-tour="stepper"]');
  const viewport = local.find((step) => step.element === '[data-tour="viewport"]');
  return [
    stepper ?? {
...
function twinDockSteps(language: "ko" | "en"): DriveStep[] {
  const local = stepsFromLocalTwinCatalog(language);
  const docks = local.filter(
    (step) =>
      step.element === '[data-tour="left-dock"]' ||
      step.element === '[data-tour="right-dock"]',
  );
...
function mergeSteps(groups: DriveStep[][]): DriveStep[] {
  const seen = new Set<string>();
  const merged: DriveStep[] = [];
  for (const group of groups) {
    for (const step of group) {
      const key = typeof step.element === "string" ? step.element : "";
...
/**
 * First-visit workspace tour plus replayable guided navigation.
 * Twin copy names the four real stages (STAGE_ORDER) and the docks.
 */
export function useOnboardingTour({
      ...
      try {
        ...
        const steps =
          kind === "basic"
            ? basicSteps(language)
            : kind === "twin"
              ? twinDockSteps(language)
              : mergeSteps([
                  basicSteps(language),
                  stepsFromLocalTwinCatalog(language),
                  twinDockSteps(language),
                ]);
        const config: Config = {
          showProgress: true,
  ...
  }, [hasSeenTour, hydrated, stage, startTour]);
