...
export const DUPLEX_ASSUMPTIONS: readonly DuplexAssumption[] = Object.freeze([
  ...
  { id: "A-WWR-ENGINE-MEAN", assumes: "All four cardinal ratios handed to MaterialProperties are the single whole-building 0.1893, not the four measured per-sector ratios.", why: "This building is where that choice stops being cosmetic. Its measured ratios are genuinely uneven — N 0.3567, E 0.1037, S 0.3667, W 0.1051, because the two mirrored dwellings glaze the short N and S elevations and not the long E and W ones — and heat-loss.ts:109 takes the UNWEIGHTED arithmetic mean of the four cardinals and multiplies the whole gross wall by it. That mean is 0.2331 against an area-weighted 0.1893: feeding the honest per-sector figures to the engine as it stands would price 340.58 × 0.2331 = 79.39 m² of window against the 64.46 m² actually measured, inflating the window area by 23 % and taking the same 14.93 m² off the opaque wall. On Schependomlaan the comparison shows nothing because its per-sector glazing is a uniform placeholder, and on the Clinic it is 2.7 %; here it is the largest single modelling choice in the file after the constructions. One ratio is applied and the eight measured ones are exported in DUPLEX_WWR_BY_SECTOR for the legend. An area-weighted mean in heat-loss.ts would let the split be consumed as measured, and DUPLEX_WWR_AREA_WEIGHTED already computes the value it should produce." },
