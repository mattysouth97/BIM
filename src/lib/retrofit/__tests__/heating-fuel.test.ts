...
describe("district-heated HVAC measures (P1-03)", () => {
  ...
  it("boiler upgrade and HRV are priced at district-heating rates with explicit fuel", () => {
    ...
    // Fuel-side form (audit #1): D × (1/η − 1/0.95) × district-heat tariff
    expect(boiler.annualCostSaving).toBeCloseTo(D * (1 / 0.65 - 1 / 0.95) * 90, 2);
    ...
    expect(hrv.annualCostSaving).toBeCloseTo((0.15 * D) / 0.65 * 90, 4);
