...
describe("reviewed corpus publication",()=>{
  ...
  it("defines the actual gross site-energy scope independently of PV credit",()=>{for(const key of ["energy.siteKwh","energy.siteKwhPerSqm"])expect(RECORD_FIELDS.find(f=>f.path===key)?.description).toContain("gross annual end-use site energy before PV");});
