...
export function BuildingTab({ buildingPk }: BuildingTabProps) {
  ...
  const validateFootprint = useCallback(
    (v: number) => (v < 4 ? t("일반적인 최소값 미만", "Below typical minimum") : null),
    [t]
  );
  const validateFloorHeight = useCallback(
    (v: number) => (v < 2.2 ? t("건축법 최소 층고(2.2m) 미만", "Below Korean code minimum (2.2m)") : null),
    [t]
  );
  const validateWindowRatio = useCallback(
    (v: number) =>
      v > 80
        ? t("한국 건축법 창면적비 제한(80%) 초과", "Exceeds Korean code limit (80%)")
        : v > 60
  ...
  const validateColumnSpacing = useCallback(
    (v: number) =>
      v < 2 ? t("최소 기둥 간격 미만", "Below minimum column spacing") : null,
    [t]
  );
  const validateColumnSize = useCallback(
    (v: number) =>
      v < 0.15 ? t("최소 기둥 크기 미만", "Below minimum column size") : null,
    [t]
  );
