...
// currently-selected scenario. Headline = NPV (oversized serif), grade chip
// = IRR band (A/B/C), caliper = year-by-year cumulative discounted cash
// flow, payback year highlighted as a vertical pin.
...
function gradeFromIrr(
  ...
  if (irr === null || irr === undefined || !Number.isFinite(irr)) {
    return { letter: "—", tone: "bad", label: "No real IRR" };
  }
  if (irr >= 0.15) return { letter: "A", tone: "good", label: "Strong" };
  if (irr >= 0.08) return { letter: "B", tone: "ok", label: "Acceptable" };
  if (irr >= 0.05) return { letter: "C", tone: "ok", label: "At hurdle" };
  return { letter: "D", tone: "bad", label: "Below hurdle" };
}
    ...
    >
      <div className="flex items-center justify-between px-4 pt-3 pb-1 border-b border-[#24282d]/70">
        <span className="text-[9px] tracking-[0.22em] uppercase text-zinc-500 font-mono">
          NPV @ {(discountRate * 100).toFixed(0)}% / {horizon}yr
        </span>
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-end gap-3 leading-none">
          <span
            className={cn(
              "text-[56px] font-light tabular-nums tracking-[-0.04em]",
              npv >= 0 ? "text-zinc-50" : "text-[#f4a765]",
            )}
            style={{
        ...
        <div className="mt-3 flex items-center gap-2">
          <span
            className={cn(
              ...
              grade.tone === "good" &&
                "border-[#8de6f3]/45 bg-[#8de6f3]/12 text-[#c7f3fb]",
              grade.tone === "ok" &&
                "border-[#fcd58a]/45 bg-[#fcd58a]/12 text-[#fcd58a]",
              grade.tone === "bad" &&
                "border-[#f4a765]/45 bg-[#f4a765]/12 text-[#f4a765]",
            )}
          >
            {grade.letter}
          </span>
        ...
        <div className="mt-1 text-[10px] font-mono text-zinc-500">
          discounted payback ·{" "}
          {Number.isFinite(payback) ? `${payback.toFixed(1)} years` : "never"}
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-[#24282d]/70 bg-[#0a0c0f]/60">
        <div className="flex items-center justify-between text-[9px] tracking-[0.2em] uppercase text-zinc-500 font-mono mb-2">
          <span>cum. discounted cash flow</span>
          <span className="text-zinc-400">year 1 → {horizon}</span>
        </div>

        <div className="relative h-12" aria-hidden="true">
          {/* Zero line — positioned by where 0 falls within [minVal, maxVal]. */}
          <div
            className="absolute inset-x-0 h-px bg-[#3a3f44]"
            style={{ top: `${zeroPct}%` }}
          />
          {/* 5-year tick marks */}
          {Array.from({ length: Math.floor(horizon / 5) + 1 }).map((_, i) => (
            <div
              key={i}
              className="absolute top-0 bottom-0 w-px bg-[#23262a]"
          ...
          {cumulativeDiscounted.map((v, t) => {
            ...
            return (
              <div
                key={t}
                className={cn("absolute", isPositive ? "bg-[#8de6f3]" : "bg-[#f4a765]")}
                style={{
                  left: `${xPct}%`,
                  width: `${widthPct}%`,
                  top,
          ...
          {Number.isFinite(payback) && payback <= horizon && (
            <div
              ...
              title={`Discounted payback: year ${payback.toFixed(1)}`}
            />
          )}
        </div>
